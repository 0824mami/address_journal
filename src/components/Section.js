...
const Section = ({ mode }) => {
  ...
  const handleInitialSubmit = () => {
    if (achieved.trim() === '' && feel.trim() === '') return;

    // エントリをオブジェクトとして作成
    const newEntry = { achieved, feel };
  ...
  return (
    <section className='section' style={sectionStyle}>
      ...
      <div className='journalArea'>
        <div>
          <input
            placeholder={placeholderAchieved}
            value={achieved}
      ...
      <div className='journalEntries'>
        {journalEntries.map((entry, index) => (
          <div key={index} style={{ marginTop: '10px' }}>
            <p><strong>{mode === 'good' ? 'たっせい! :' : 'うんうん :'}</strong> {entry.achieved}</p>
